  ...
  try {
    ...
    const result = await runIdempotentTransaction(idempotency, async (tx) => {
      ...
      await writeAuditLog(tx, {
        ...
        change: { action: "PASSWORD_RESET", targetType: "USER", targetId: id, summary: target.protectedSystemAdmin ? "Protected Admin password rotated and sessions revoked" : "Local password reset and sessions revoked" },
      });
    ...
    const code = message === "NOT_FOUND" ? "FORBIDDEN" : "INTERNAL_ERROR";
    return NextResponse.json(failure(code, "密码重置失败", context.requestId), { status });
  }
}
