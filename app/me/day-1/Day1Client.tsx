    ...
    <main className="ow-phone ow-enter">
      ...
      <div className="student-collage mt-8">
        {data.template.slots.map((config, index) => {
          ...
          return <button id={`slot-${config.slotKey}`} key={config.slotKey} type="button" disabled={readOnly || uploading} onClick={() => failed ? setActiveCrop(failed) : value?.imageUrl ? setActiveCrop({ config, value, preview: value.imageUrl }) : choose(config)} className={`student-slot student-slot-${index % 6} ${missing.includes(config.slotKey) ? "student-slot-missing" : ""}`} style={{ aspectRatio: config.aspectRatio }}><span className="student-slot-media">{imageUrl ? <ResilientImage src={imageUrl} alt="" eager={index < 3} style={{ transform: `translate(${(crop.x - .5) * 36}%, ${(crop.y - .5) * 36}%) scale(${crop.scale})` }} /> : <span className="student-slot-add" aria-hidden="true">＋</span>}</span>{uploading && <span className="absolute top-2 right-2 z-10 max-w-[85%] rounded-full bg-white/95 px-2 py-1 text-[11px] font-black text-[var(--orange)]">{uploadStages[config.slotKey] || "处理中…"}</span>}{failed && <span className="absolute top-2 right-2 z-10 rounded-full bg-red-700 px-2 py-1 text-[11px] font-black text-white">{failed.failureKind === "too-large" ? "请换一张" : "点按重试"}</span>}<span className="student-slot-label">{config.label}{config.required ? " *" : ""}</span></button>;
        })}
      </div>
