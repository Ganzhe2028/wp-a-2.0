...
function CropDialog({ source, aspectRatio, initial, cancel, replace, accept }: { source: string; aspectRatio: number; initial: Crop; cancel: () => void; replace: () => void; accept: (crop: Crop) => void }) {
  ...
  function setScale(scale: number) { setCrop((current) => ({ ...current, scale: Math.min(3, Math.max(1, +scale.toFixed(2))) })); }
  function move(event: React.PointerEvent<HTMLDivElement>) {
    ...
    setCrop({
      ...start.crop,
      x: Math.min(1, Math.max(0, start.crop.x + (event.clientX - start.x) / rect.width)),
      y: Math.min(1, Math.max(0, start.crop.y + (event.clientY - start.y) / rect.height)),
    });
  }
  function stopDragging(event: React.PointerEvent<HTMLDivElement>) {
    if (dragging.current?.id === event.pointerId) dragging.current = null;
  }
  return <ViewportDialog close={cancel}><div className="ow-modal student-dialog" role="dialog" aria-modal="true" aria-labelledby="crop-title"><h2 id="crop-title" className="text-2xl font-black">调整图片位置</h2><p className="ow-muted mt-2">100% 会显示完整图片。放大后拖动图片，选择你想展示的画面。</p><div className="student-crop-window mt-6" style={{ aspectRatio }} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); dragging.current = { id: event.pointerId, x: event.clientX, y: event.clientY, crop }; }} onPointerMove={move} onPointerUp={stopDragging} onPointerCancel={stopDragging}><img src={source} alt="待裁切图片完整预览" draggable={false} style={{ transform: `translate(${(crop.x - .5) * 36}%, ${(crop.y - .5) * 36}%) scale(${crop.scale})` }} /></div><div className="mt-5 flex items-center justify-center gap-3" aria-label="缩放图片"><button type="button" className="student-square-button" aria-label="缩小" disabled={crop.scale <= 1} onClick={() => setScale(crop.scale - .1)}>−</button><span className="min-w-14 text-center font-bold">{Math.round(crop.scale * 100)}%</span><button type="button" className="student-square-button" aria-label="放大" disabled={crop.scale >= 3} onClick={() => setScale(crop.scale + .1)}>＋</button></div><label className="sr-only" htmlFor="day1-crop-scale">图片缩放比例</label><input id="day1-crop-scale" type="range" min="1" max="3" step="0.05" value={crop.scale} onChange={(event) => setScale(Number(event.target.value))} className="mt-4 w-full accent-[var(--orange)]" /><button type="button" onClick={() => setCrop(EMPTY_CROP)} className="mt-3 min-h-11 w-full text-sm font-bold underline">恢复完整图片</button><button type="button" onClick={() => accept(crop)} className="ow-btn mt-4">使用这个位置</button><button type="button" onClick={replace} className="ow-btn ow-btn-outline mt-3">重新选择图片</button><button type="button" onClick={cancel} className="mt-3 min-h-11 w-full font-bold">取消</button></div></ViewportDialog>;
}
