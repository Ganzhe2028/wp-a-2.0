...
export default function Day1Client() {
  ...
  async function acceptCrop(crop: Crop) {
    ...
    try {
      const compressed = await imageCompression(file, { maxSizeMB: .45, maxWidthOrHeight: 1600, useWebWorker: true, fileType: "image/webp", initialQuality: .82 });
      ...
      const presigned = await studentApi<{ assetId: string; uploadUrl: string }>("/api/v1/assets/presign", { method: "POST", headers: { "Idempotency-Key": newIdempotencyKey() }, body: JSON.stringify({ section: "DAY1", fileName: file.name, mimeType: compressed.type, byteSize: compressed.size, checksum }) });
      const uploaded = await fetch(presigned.uploadUrl, { method: "PUT", body: compressed, headers: { "Content-Type": compressed.type } });
      if (!uploaded.ok) throw new Error("图片上传失败，请重试");
      const completed = await studentApi<{ assetId?: string; imageUrl?: string; url?: string; status?: string }>(`/api/v1/assets/${encodeURIComponent(presigned.assetId)}/complete`, { method: "POST", headers: { "Idempotency-Key": newIdempotencyKey() }, body: JSON.stringify({ section: "DAY1" }) });
      let imageUrl = completed.imageUrl || completed.url;
      if (!imageUrl && completed.status === "PROCESSING") {
        for (let attempt = 0; attempt < 90; attempt += 1) {
          await new Promise((resolve) => window.setTimeout(resolve, attempt < 12 ? 750 : 1_500));
          const status = await studentApi<{ status: "PROCESSING" | "READY" | "FAILED"; imageUrl?: string }>(`/api/v1/assets/${encodeURIComponent(presigned.assetId)}`);
  ...
  return (
    <main className="ow-phone ow-enter">
      ...
      {uploadingSlots.size > 0 && <div className="student-notice mt-5" role="status"><b>后台正在处理 {uploadingSlots.size} 张图片</b><p>无需停在这里等待，可以继续选择和裁切其他照片；全部完成后即可提交。</p></div>}
      ...
      <div className="student-collage mt-8">
        {data.template.slots.map((config, index) => {
          ...
          return <button id={`slot-${config.slotKey}`} key={config.slotKey} type="button" disabled={readOnly || uploading} onClick={() => failed ? setActiveCrop(failed) : value?.imageUrl ? setActiveCrop({ config, value, preview: value.imageUrl }) : choose(config)} className={`student-slot student-slot-${index % 6} ${missing.includes(config.slotKey) ? "student-slot-missing" : ""}`} style={{ aspectRatio: config.aspectRatio }}><span className="student-slot-media">{imageUrl ? <img src={imageUrl} alt="" style={{ transform: `translate(${(crop.x - .5) * 36}%, ${(crop.y - .5) * 36}%) scale(${crop.scale})` }} /> : <span className="student-slot-add" aria-hidden="true">＋</span>}</span>{uploading && <span className="absolute top-2 right-2 z-10 rounded-full bg-white/95 px-2 py-1 text-[11px] font-black text-[var(--orange)]">处理中…</span>}{failed && <span className="absolute top-2 right-2 z-10 rounded-full bg-red-700 px-2 py-1 text-[11px] font-black text-white">点按重试</span>}<span className="student-slot-label">{config.label}{config.required ? " *" : ""}</span></button>;
        })}
      </div>
