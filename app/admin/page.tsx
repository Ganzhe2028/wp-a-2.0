...
function Login({ onLogin }: { onLogin: () => void }) {
  ...
  return <main className="flex min-h-svh items-center justify-center bg-[var(--paper)] p-5"><form onSubmit={submit} className="w-full max-w-sm border-2 border-black bg-white p-8"><h1 className="text-3xl font-black">O—WEEK / ADMIN</h1>{localAdminPassword && <section className="mt-6 border-2 border-[var(--orange)] bg-[var(--orange-soft)] p-4 text-sm"><p className="font-black text-[var(--orange)]">本地管理员凭据</p><p className="mt-3"><span className="font-bold">账号：</span>无需账号（共享口令登录）</p><p className="mt-1 break-all"><span className="font-bold">密码：</span><code>{localAdminPassword}</code></p><p className="mt-3 text-xs text-neutral-600">仅在开发环境显示，生产环境不会输出。</p></section>}<label className="mt-8 block font-bold">管理口令<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-2 min-h-12 w-full border-2 border-black px-3 text-base" /></label>{error && <p className="mt-3 text-red-600">{error}</p>}<button disabled={loading || !password} className="ow-btn mt-6">{loading ? "验证中…" : "登录"}</button></form></main>;
}
...
function ImportDialog({ close, refresh }: { close: () => void; refresh: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-5"><section className="w-full max-w-2xl bg-white p-6"><div className="flex justify-between"><h2 className="text-2xl font-black">导入账号</h2><button onClick={close} className="h-11 w-11 text-3xl text-[var(--orange)]">×</button></div>{result.length === 0 ? <><p className="mt-4 text-sm text-neutral-500">每行：中文名,英文名,组别,角色,可选用户名</p><textarea value={text} onChange={(event) => setText(event.target.value)} rows={10} className="mt-4 w-full border-2 border-black p-3" placeholder="林若安,Ruoan,Group 02,LEARNER" />{error && <p className="mt-3 text-red-600">{error}</p>}<button onClick={submit} disabled={loading || !text.trim()} className="ow-btn mt-5">{loading ? "导入中…" : "确认导入"}</button></> : <><p className="mt-4 font-bold text-emerald-700">成功导入 {result.length} 个账号。密码仅显示一次。</p><div className="mt-4 max-h-80 overflow-auto border border-neutral-300">{result.map((account) => <div key={account.username} className="grid grid-cols-3 border-b p-3 text-sm"><b>{account.chineseName}</b><span>{account.username}</span><code>{account.password}</code></div>)}</div><button onClick={close} className="ow-btn mt-5">完成</button></>}</section></div>;
}
