...
export default function Day3Editor({ initialAnswers, submitted, allowEdit = false }: { initialAnswers: Day3Answers; submitted: boolean; allowEdit?: boolean }) {
  ...
  const readOnly = submitted && !allowEdit;
  ...
  const save = useCallback(async (action: "saveDay3" | "submitDay3") => {
    ...
    if (response.status === 401) { setExpired(true); setSaving(false); return false; }
  ...
  if (readOnly) return <main className="ow-phone ow-enter"><OweekHeader title="MY PROFILE" action="HOME" actionHref="/home" /><div className="flex items-center justify-between"><h1 className="ow-heading">DAY 3 已提交</h1><span className="ow-chip ow-chip-active">SUBMITTED</span></div><p className="ow-muted mt-3">你已解锁 Browse 中其他人的 Day 3。</p><div className="mt-10 grid grid-cols-2 gap-4">{DAY3_SECTIONS.map((item, s) => <div key={item.title} className="rounded-[var(--radius-card)] border-[1.5px] border-[var(--orange)]"><div className="grid grid-cols-4 gap-2 p-4">{answers[s].slice(0, 16).map((level, i) => <span key={i} className="scale-75"><Bottle level={level} /></span>)}</div><b className="block rounded-b-[calc(var(--radius-card)-2px)] bg-black p-3 text-white">{item.title.replace("的小瓶子", "")} · 32 项</b></div>)}</div><div className="mt-28 rounded-[var(--radius-card)] border-[1.5px] border-[var(--orange)] bg-[var(--orange-soft)] p-6"><b className="text-2xl">✓ 已发布 · 只读展示</b><p className="ow-muted mt-2">提交后内容由管理员开放编辑时才可修改。</p></div><Link href="/browse" className="ow-btn mt-24">去 Browse 看看 →</Link><Link href="/home" className="ow-btn ow-btn-outline mt-3">返回首页</Link></main>;
  ...
  return (
    <main className="ow-phone ow-enter">
      ...
      {sectionIndex === 0 ? <button onClick={() => { setSectionIndex(1); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="ow-btn mt-14">下一页：安心小瓶子 →</button> : submitted ? <p className="ow-chip ow-chip-active mt-14 w-full">已提交 · 修改会自动保存</p> : <button onClick={() => setConfirming(true)} className="ow-btn mt-14">提交 DAY 3 →</button>}
      ...
      {confirming && <div className="ow-scrim fixed inset-0 z-50 flex items-center justify-center p-5" role="dialog" aria-modal="true"><div className="ow-modal w-full max-w-xl p-6"><h2 className="ow-heading">要提交 DAY 3 吗？</h2><p className="ow-muted mt-5 text-lg leading-8">你选择了 {total} 个小瓶子。未选择的项目会保持为空，提交后进入只读展示。</p><div className="mt-8 border-[1.5px] border-[var(--orange)] bg-[var(--orange-soft)] p-5 font-bold">已选择 {total} / 64 · 草稿已保存</div><button onClick={submit} disabled={saving} className="ow-btn mt-8">确认提交</button><button onClick={() => setConfirming(false)} className="ow-btn ow-btn-outline mt-3">返回检查</button></div></div>}
    </main>
  );
}
