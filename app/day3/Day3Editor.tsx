...
export default function Day3Editor({ initialAnswers, submitted }: { initialAnswers: Day3Answers; submitted: boolean }) {
  ...
  return (
    <main className="ow-phone ow-enter">
      ...
      {sectionIndex === 0 ? <button onClick={() => { setSectionIndex(1); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={sectionComplete < 32} className="ow-btn mt-14">下一页：安心小瓶子 →</button> : <button onClick={() => setConfirming(true)} disabled={total < 64} className="ow-btn mt-14">提交 DAY 3 →</button>}
      ...
      {confirming && <div className="ow-scrim fixed inset-0 z-50 flex items-center justify-center p-5" role="dialog" aria-modal="true"><div className="ow-modal w-full max-w-xl p-6"><h2 className="ow-heading">要提交 DAY 3 吗？</h2><p className="ow-muted mt-5 text-lg leading-8">64 / 64 个小瓶子选项已完成。提交后进入只读展示。</p><div className="mt-8 border-2 border-[var(--orange)] bg-[var(--orange-soft)] p-5 font-bold">完成度 100% · 草稿已保存</div><button onClick={submit} disabled={saving} className="ow-btn mt-8">确认提交</button><button onClick={() => setConfirming(false)} className="ow-btn ow-btn-outline mt-3">返回检查</button></div></div>}
    </main>
  );
}
