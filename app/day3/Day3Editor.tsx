...
export default function Day3Editor({ initialAnswers, submitted, allowEdit = false }: { initialAnswers: Day3Answers; submitted: boolean; allowEdit?: boolean }) {
  ...
  if (readOnly) return <main className="ow-phone ow-enter"><OweekHeader title="MY PROFILE" action="HOME" actionHref="/home" /><div className="flex items-center justify-between"><h1 className="ow-heading">DAY 3 已提交</h1><span className="ow-chip ow-chip-active">SUBMITTED</span></div><p className="ow-muted mt-3">你已解锁 Browse 中其他人的 Day 3。</p><div className="mt-10 grid grid-cols-2 gap-4">{DAY3_SECTIONS.map((item, s) => <div key={item.title} className="rounded-[var(--radius-card)] border-2 border-[var(--orange)]"><div className="grid grid-cols-4 gap-2 p-4">{answers[s].slice(0, 16).map((level, i) => <span key={i} className="scale-75"><Bottle level={level} /></span>)}</div><b className="block rounded-b-[calc(var(--radius-card)-2px)] bg-black p-3 text-white">{item.title.replace("的小瓶子", "")} · 32 项</b></div>)}</div><div className="mt-28 rounded-[var(--radius-card)] border-2 border-[var(--orange)] bg-[var(--orange-soft)] p-6"><b className="text-2xl">✓ 已发布 · 只读展示</b><p className="ow-muted mt-2">提交后内容由管理员开放编辑时才可修改。</p></div><Link href="/browse" className="ow-btn mt-24">去 Browse 看看 →</Link><Link href="/home" className="ow-btn ow-btn-outline mt-3">返回首页</Link></main>;
