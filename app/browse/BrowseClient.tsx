...
export default function BrowseClient({ people, day1Unlocked, day3Unlocked, showNames, forceGate }: { people: Person[]; day1Unlocked: boolean; day3Unlocked: boolean; showNames: boolean; forceGate: boolean }) {
  ...
  function PersonRow({ person }: { person: Person }) {
    ...
    return <Link href={`/u/${person.code}?day=${day}`} className="flex min-h-20 items-center border-2 border-[var(--line)] px-4"><span className="mr-4 h-12 w-12 shrink-0 rounded-full bg-black" /><span><b className="text-xl">{displayName}</b><small className="ow-muted mt-1 block">{person.groupName || (person.role === "SENIOR" ? "Senior Group" : "Learner")}</small></span><span className="ow-orange ml-auto text-4xl">→</span></Link>;
  }
