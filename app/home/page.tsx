  ...
  return (
    <main className="ow-phone ow-enter">
      ...
      <div className="mt-8 space-y-4">
        ...
        <Link href={day3Open ? "/day3" : "/home"} aria-disabled={!day3Open} className={`block min-h-36 border-2 border-[var(--orange)] bg-[var(--orange)] p-5 ${!day3Open ? "opacity-40" : ""}`}><div className="flex items-center justify-between font-bold"><b>03</b><span className="ow-chip ow-chip-white text-sm">{!day3Open ? "未开放" : day3Action}</span></div><div className="mt-4 text-5xl font-black">DAY 3</div><p className="mt-2">小瓶子 · {day3Progress} / 64</p><div className="ow-arrow text-right text-4xl">→</div></Link>
        <Link href={browseOpen ? (browseUnlocked ? "/browse" : "/browse?gate=1") : "/home"} aria-disabled={!browseOpen} className={`ow-card block min-h-36 p-5 ${!browseOpen ? "opacity-40" : ""}`}><div className="flex items-center justify-between"><b className="ow-arrow ow-muted">↗</b><span className="ow-chip text-sm">{!browseOpen ? "未开放" : browseUnlocked ? "已解锁" : "未解锁"}</span></div><div className="mt-4 text-5xl font-black">BROWSE</div><p className="ow-muted mt-2">浏览 Senior Group 与 Learners</p><div className="ow-arrow ow-orange text-right text-4xl">→</div></Link>
      </div>
