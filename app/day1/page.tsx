...
export default async function Day1Page() {
  ...
  const [person, allowEdit] = await Promise.all([
    prisma.person.findUnique({ where: { id: session.personId }, select: { code: true, avatarUrl: true, day1SubmittedAt: true, images: { where: { hidden: false }, orderBy: { sort: "asc" }, select: { id: true, url: true, sort: true } } } }),
    settingEnabled("allowEdit", false),
  ]);
  if (!person) redirect("/");
  return <Day1Editor person={person} allowEdit={allowEdit} />;
}
